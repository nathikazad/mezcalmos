...
SELECT
  ...
  (SELECT id FROM public.user u WHERE phone_number = m.phone_number) AS user_id,
  ...
  CASE
    ...
    WHEN (
        SELECT COUNT(*)
        FROM delivery_messages dm2
        WHERE dm2.phone_number = m.phone_number AND dm2.finished_time IS NULL AND dm2.driver_id IS NOT NULL
    ) > 0 THEN 'inProcess'
    ELSE 'finished'
END AS status
