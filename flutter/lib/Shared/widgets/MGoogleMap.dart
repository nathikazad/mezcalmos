...
class MGoogleMapState extends State<MGoogleMap> {
  ...
  Widget build(BuildContext context) {
    return Obx(
      () {
        mezDbgPrint(
            "the length of markers ${widget.mGoogleMapController.markers.length} from MGoogleMapWidget");
        return Stack(
          children: <Widget>[
  ...
  Positioned locateMeButton(BuildContext context) {
    return Positioned(
      right: 10,
      bottom: 10,
      child: InkWell(
        ...
        child: Container(
          height: MezCalmosResizer.isDesktop(context) ||
                  MezCalmosResizer.isSmallTablet(context)
              ? 50
              : 10.w,
          width: MezCalmosResizer.isDesktop(context) ||
                  MezCalmosResizer.isSmallTablet(context)
              ? 50
              : 10.w,
