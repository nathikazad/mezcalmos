...
Future<Operator?> get_delivery_operator({required int userId}) async {
  ...
  if (res.parsedData!.delivery_operator.isNotEmpty) {
    ...
    return Operator(
        state: OperatorState(
            deliveryDetailsId: data.delivery_company.delivery_details_id,
            serviceProviderDetailsId: data.delivery_company.details_id,
            serviceLinkId: data.delivery_company.details!.service_link_id,
  ...
  return _db.graphQLClient
    ...
    if (event.parsedData?.delivery_operator == null ||
      ...
      if (Get.find<HasuraDb>()
          .dataConsumption
          .containsKey("dvOperatorStatusStream")) {
        Get.find<HasuraDb>().dataConsumption["dvOperatorStatusStream"]![0] +=
            event.data?.toString().length ?? 0;
        Get.find<HasuraDb>().dataConsumption["dvOperatorStatusStream"]![1] += 1;
      } else {
        Get.find<HasuraDb>().dataConsumption["dvOperatorStatusStream"] = <int>[
          event.data?.toString().length ?? 0,
          1
        ];
      }
...
Future<List<Operator>?> get_delivery_company_operators(
  ...
  return data
      .map((Query$getCompanyOerators$delivery_operator opData) => Operator(
          state: OperatorState(
              serviceLinkId: opData.delivery_company.details!.service_link_id,
              deliveryDetailsId: opData.delivery_company.delivery_details_id,
