...
Future<int> set_cart_business_id({
  ...
  final QueryResult<Mutation$insert_business_cart> _cart =
      await _hasuraDb.graphQLClient.mutate$insert_business_cart(
    Options$Mutation$insert_business_cart(
      fetchPolicy: FetchPolicy.noCache,
      variables: Variables$Mutation$insert_business_cart(
  ...
  mezDbgPrint(
      "[🛑] called :: set_cart_business_id :: success :: ${_cart.data}");

  if (_cart.parsedData?.insert_business_cart_one == null ||
      _cart.parsedData!.insert_business_cart_one?.business_id == null) {
    throw Exception(
        "[🛑] called :: set_cart_business_id :: exception :: ${_cart.hasException}");
  } else {
    final int newRestId =
        _cart.parsedData!.insert_business_cart_one!.business_id!;
