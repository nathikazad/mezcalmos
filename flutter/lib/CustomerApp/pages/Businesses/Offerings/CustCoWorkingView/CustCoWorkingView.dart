...
import 'package:mezcalmos/CustomerApp/pages/Businesses/Offerings/CustCoWorkingView/controllers/CustCoWorkingViewController.dart';
...
class CustCoWorkingView extends StatefulWidget {
  const CustCoWorkingView({super.key});
  static Future<void> navigate({
    ...
    final String route =
        CustBusinessRoutes.custCoWorkingRoute.replaceFirst(":id", "$rentalId");
  ...
  @override
  State<CustCoWorkingView> createState() => _CustCoWorkingViewState();
}

class _CustCoWorkingViewState extends State<CustCoWorkingView> {
  CustCoWorkingViewController viewController = CustCoWorkingViewController();
  int? rentalId;
  @override
  void initState() {
  ...
  Column _coWorkingType(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        SizedBox(
        ...
        if (viewController
                .homeRental?.details.additionalParameters?['roomType1'] ==
            CoWorkingRoomType.DedicatedDesk.toFirebaseFormatString())
          Column(children: <Widget>[
            Text(
              'Co-working type',
              style: context.textTheme.bodyLarge?.copyWith(fontSize: 10.mezSp),
            ),
          ]),
        smallSepartor,
        if (viewController
                .homeRental?.details.additionalParameters?['roomType1'] ==
            CoWorkingRoomType.FullFloorOffice.toFirebaseFormatString())
          Column(children: <Widget>[
            RichText(
              text: TextSpan(
                  text: 'Private room',
                  style:
                      context.textTheme.bodyLarge?.copyWith(fontSize: 11.mezSp),
                  children: [
                    WidgetSpan(child: hSmallSepartor),
                    TextSpan(
                        text: '\$144/day',
                        style: context.textTheme.bodyLarge?.copyWith(
                            fontSize: 11.mezSp, color: primaryBlueColor))
                  ]),
            ),
          ]),
        if (viewController
                .homeRental?.details.additionalParameters?['roomType1'] ==
            CoWorkingRoomType.PrivateRoom.toFirebaseFormatString())
          Text('Three'),
      ],
    );
  }
