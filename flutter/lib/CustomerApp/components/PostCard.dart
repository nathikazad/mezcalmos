...
class PostCard extends StatefulWidget {
  ...
  const PostCard(
      {required this.post, required this.onLike, required this.onPostComment});
...
class _PostCardState extends State<PostCard> {
          ...
          children: [
            Row(
              children: [
                CircleAvatar(
                  backgroundImage:
                      NetworkImage(widget.post.serviceProviderImage ?? ''),
                  radius: 25,
                ),
                SizedBox(width: 8),
                Column(
                  ...
                  children: [
                    ...
                    Text(
                      widget.post.postedOn.timeAgo(numericDates: false),
            ...
            if (_comments.length > 3) SizedBox()
          ],
        ),
      ),
    );
  ...
  Widget _buildCommentsList() {
    final List<Comment> commentsToShow = _comments.reversed.take(3).toList();
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        SizedBox(height: 8),
