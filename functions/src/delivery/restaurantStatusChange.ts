...
export async function deliveryDriverAtPickup(
  userId: number,
  data: ChangeDeliveryStatusDetails
) {
  let response: ServerResponse = await changeStatus(
    data,
    DeliveryOrderStatus.AtPickup,
    userId
  );
...
export async function deliveryDriverAtDropoff(
  userId: number,
  data: ChangeDeliveryStatusDetails
) {
  let response: ServerResponse = await changeStatus(
...
function checkExpectedStatus(
  ...
  if (
    newStatus == DeliveryOrderStatus.AtPickup &&
    currentStatus != DeliveryOrderStatus.OrderReceived &&
    currentStatus != DeliveryOrderStatus.PackageReady
  ) {
    throw new HttpsError("internal", "invalid delivery order status");
  } else if (
    currentStatus !=
    statusArrayInSeq[
...
async function changeStatus(
  ...
  //   }
  // }
  let deliveryOrderPromise = getDeliveryOrder(
    changeDeliveryStatusDetails.deliveryId
  );
  let deliveryDriverPromise = getDeliveryDriver(
    changeDeliveryStatusDetails.deliveryDriverId,
    changeDeliveryStatusDetails.deliveryDriverType
  );
  let restaurantOrderPromise = getRestaurantOrder(
    changeDeliveryStatusDetails.restaurantOrderId
  );
  let promiseResponse = await Promise.all([
    deliveryOrderPromise,
    deliveryDriverPromise,
    restaurantOrderPromise,
  ]);
  ...
  let restaurantOperators: RestaurantOperator[] = await getRestaurantOperators(
  ...
  if (
    ...
    throw new HttpsError("internal", "delivery order is complete or cancelled");
  }
  if (deliveryDriver.userId != userId) {
    throw new HttpsError("internal", "invalid delivery driver user id");
  }
