...
export async function getDeliveryOrder(
  ...
  let delivery: DeliveryOrder = {
    ...
    pickupLocation:
      response.delivery_order_by_pk.pickup_gps &&
      response.delivery_order_by_pk.pickup_gps.coordinates &&
      response.delivery_order_by_pk.pickup_address
        ? {
    ...
    chatWithServiceProviderId:
      response.delivery_order_by_pk.chat_with_service_provider_id,
    ...
    deliveryCost: parseFloat(
      response.delivery_order_by_pk.delivery_cost.replace("$", "")
    ),
    packageCost: parseFloat(
      response.delivery_order_by_pk.package_cost.replace("$", "")
    ),
    orderTime: response.delivery_order_by_pk.order_time,
    serviceProviderType: response.delivery_order_by_pk
    ...
    deliveryDriver: response.delivery_order_by_pk.delivery_driver
      ? {
          ...
          deliveryCompanyType: response.delivery_order_by_pk.delivery_driver
            .delivery_company_type as DeliveryServiceProviderType,
          deliveryCompanyId:
            response.delivery_order_by_pk.delivery_driver.delivery_company_id,
          status: response.delivery_order_by_pk.delivery_driver
          ...
          user: {
            ...
            firebaseId:
              response.delivery_order_by_pk.delivery_driver.user.firebase_id,
            language: response.delivery_order_by_pk.delivery_driver.user
              .language_id as Language,
          },
          notificationInfo: response.delivery_order_by_pk.delivery_driver
            .notification_info
            ? {
                appType: AppType.Delivery,
                token:
                  response.delivery_order_by_pk.delivery_driver
                    .notification_info.token,
                turnOffNotifications:
                  response.delivery_order_by_pk.delivery_driver
                    .notification_info.turn_off_notifications,
              }
            : undefined,
        }
      : undefined,
        ...
        ? {
            ...
            notificationInfo: d.delivery_driver.notification_info
              ? {
                  ...
                  turnOffNotifications:
                    d.delivery_driver.notification_info.turn_off_notifications,
                }
              : undefined,
          }
        : undefined,
...
export async function getDeliveryCompany(
  ...
  if (
    response.delivery_company_by_pk == null ||
    response.delivery_company_by_pk.details == null
  ) {
    throw new MezError("deliveryCompanyNotFound");
  }
  let operators: Operator[] =
    response.delivery_company_by_pk.delivery_operators.map((o: any) => {
      return {
        id: o.id,
  ...
  return {
    ...
    location: {
      lat: response.delivery_company_by_pk?.details?.location.gps
        .coordinates[1],
      lng: response.delivery_company_by_pk?.details?.location.gps
        .coordinates[0],
