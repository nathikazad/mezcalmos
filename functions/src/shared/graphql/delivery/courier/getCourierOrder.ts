...
export async function getCourierOrder(orderId: number): Promise<CourierOrder> {
    ...
    let courierOrder: CourierOrder = {
        ...
        deliveryOrder: {
            ...
            driverId: response.delivery_courier_order_by_pk.delivery_order.delivery_driver_id,
            ...
            chosenCompanies: response.delivery_courier_order_by_pk.delivery_order.chosen_companies,
            customerOffer: response.delivery_courier_order_by_pk.delivery_order.customer_offer,
            counterOffers: response.delivery_courier_order_by_pk.delivery_order.counter_offers,
            notifiedDrivers: response.delivery_courier_order_by_pk.delivery_order.notified_drivers,
...
export async function getCourierOrderByDeliveryId(deliveryOrderId: number): Promise<CourierOrder> {
    let chain = getHasura();
  
    let response =  await chain.query({
      delivery_courier_order: [
            { where: { delivery_order_id: { _eq: deliveryOrderId } } }, {
                id: true,
                cancellation_time: true,
                customer_id: true,
                discount_value: true,
    ...
    if(response.delivery_courier_order[0] == null) {
        throw new MezError("orderNotFound");
    }
    ...
    let courierOrder: CourierOrder = {
        ...
        items,
        deliveryOrder: {
            packageReady: response.delivery_courier_order[0].delivery_order.package_ready,
            deliveryId: deliveryOrderId,
            serviceProviderId: response.delivery_courier_order[0].delivery_order.service_provider_id,
            orderType: response.delivery_courier_order[0].delivery_order.order_type as OrderType,
            pickupLocation: (response.delivery_courier_order[0].delivery_order.pickup_gps && response.delivery_courier_order[0].delivery_order.pickup_address) ? {
                lat: response.delivery_courier_order[0].delivery_order.pickup_gps.coordinates[1],
                lng: response.delivery_courier_order[0].delivery_order.pickup_gps.coordinates[0],
                address: response.delivery_courier_order[0].delivery_order.pickup_address,
            }: undefined,
            dropoffLocation: {
                lat: response.delivery_courier_order[0].delivery_order.dropoff_gps.coordinates[1],
                lng: response.delivery_courier_order[0].delivery_order.dropoff_gps.coordinates[0],
                address: response.delivery_courier_order[0].delivery_order.dropoff_address,
            },
            chatWithServiceProviderId: response.delivery_courier_order[0].delivery_order.chat_with_service_provider_id,
            chatWithCustomerId: response.delivery_courier_order[0].delivery_order.chat_with_customer_id,
            paymentType: response.delivery_courier_order[0].delivery_order.payment_type as PaymentType,
            status: response.delivery_courier_order[0].delivery_order.status as DeliveryOrderStatus,
            customerId: response.delivery_courier_order[0].delivery_order.customer_id,
            deliveryCost: parseFloat(response.delivery_courier_order[0].delivery_order.delivery_cost.replace("$", "")),
            packageCost: parseFloat(response.delivery_courier_order[0].delivery_order.package_cost.replace("$", "")),
            orderTime: response.delivery_courier_order[0].delivery_order.order_time,
            serviceProviderType: response.delivery_courier_order[0].delivery_order.service_provider_type as DeliveryServiceProviderType,
            direction: response.delivery_courier_order[0].delivery_order.direction as DeliveryDirection,
            driverId: response.delivery_courier_order[0].delivery_order.delivery_driver_id,
            tripDistance: response.delivery_courier_order[0].delivery_order.trip_distance,
            tripDuration: response.delivery_courier_order[0].delivery_order.trip_duration,
            tripPolyline: response.delivery_courier_order[0].delivery_order.trip_polyline,
            chosenCompanies: response.delivery_courier_order[0].delivery_order.chosen_companies,
            customerOffer: response.delivery_courier_order[0].delivery_order.customer_offer,
            counterOffers: response.delivery_courier_order[0].delivery_order.counter_offers,
            notifiedDrivers: response.delivery_courier_order[0].delivery_order.notified_drivers,
            deliveryDriver: (response.delivery_courier_order[0].delivery_order.delivery_driver) ? {
                ...
                user: {
                    id: response.delivery_courier_order[0].delivery_order.delivery_driver.user.id,
                    firebaseId: response.delivery_courier_order[0].delivery_order.delivery_driver.user.firebase_id,
                    language: response.delivery_courier_order[0].delivery_order.delivery_driver.user.language_id as Language
                },
                notificationInfo: (response.delivery_courier_order[0].delivery_order.delivery_driver.notification_info) ? {
                    appType: AppType.Delivery,
                    token: response.delivery_courier_order[0].delivery_order.delivery_driver.notification_info.token,
                    turnOffNotifications: response.delivery_courier_order[0].delivery_order.delivery_driver.notification_info.turn_off_notifications,
                } : undefined,
            }: undefined
        }
    }
